...
vi.mock("@decky/ui", () => {
  ...
  return {
    ...
    // no Steam webpack modules, so a test that wants the class names, the
    // tabbed page or a scroll panel mocks `frontend/src/utils/deckyUiInternals`
    // with its own values. Named explicitly rather than left off: Vitest throws
    // on an import of a name the mock factory does not define, so an omission
    // would break every file that reaches @decky/ui through deckyUiInternals.
